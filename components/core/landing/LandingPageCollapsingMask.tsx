...
export default function LandingPageCollapsingMask(
    ...
    const maskBgRef = useRef<SVGRectElement | null>(null);   // new: background rect of mask
    const maskElRef = useRef<SVGMaskElement | null>(null);   // optional, the <mask> element itself
    ...
    useEffect(() => {
        ...
        maskBgRef.current = maskBg.node() as SVGRectElement;
        ...
        const onResize = () => {
            ...
            if (maskBgRef.current) {
                d3.select(maskBgRef.current)
                    .attr('width', w)
                    .attr('height', h);
            }

            // tracker center + radius
            if (trackerRef.current) {
                const t = d3.select(trackerRef.current);
                t.attr('cx', w / 2)
                    .attr('cy', h / 2)
                    .attr('r', Math.max(Number(t.attr('r')) || 0, diag));
            }

            // force mask to repaint (important on Chrome/Safari)
            if (overlayRectRef.current) {
                const sel = d3.select(overlayRectRef.current);
                sel.attr('mask', null);
                requestAnimationFrame(() => sel.attr('mask', 'url(#shrink-mask)'));
            }
        };
        ...
        return () => {
            ...
            maskBgRef.current = null;
            maskElRef.current = null;
