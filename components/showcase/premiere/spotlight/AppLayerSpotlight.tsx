...
// One-shot seed consumed from the spotlight store (Landing -> Premiere handoff)
    ...
    if (seedRef.current === null) {
        ...
        const s = api.consumeSeed?.() ?? null;
        seedRef.current = s as Seed | null;
    }
    const seed = seedRef.current;


// If a seed existed, sync the store (so other readers see it) once after mount
    useEffect(() => {
        if (!seed) return;
        const api = useSpotlightMaskStore.getState();
        api.setAll(seed);
...
// Build initial values (seed -> props -> last mask -> defaults)
    const {mask: lastMask} = useSpotlightMaskStore.getState();
    const cx0: number =
        (seed?.cx ?? lastMask?.cx) ?? vw() / 2;
    const cy0: number =
        (seed?.cy ?? lastMask?.cy) ?? vh() / 2;
    const r0: number =
        (seed?.r ?? lastMask?.r) ?? 150;
